...
class AddressConfirmationMixin:
    ...
    async def _do_generate_and_upload(
        ...
        *,
        ...
        # 从 layyxq 材料槽位数据获取 qmPath，作为 dzqmList 无数据时的兜底
        if not sig_oss_path:
            try:
                detail = await self._get(f"/yzw-zxfw-lafw/api/v3/layy/layyxq/{layyid}/0")
                if isinstance(detail, dict):
                    # 通用遍历查找 _CLLX 对应槽位的 qmPath
                    queue: list[Any] = [detail]
                    while queue:
                        node = queue.pop(0)
                        if isinstance(node, dict):
                            node_cllx = str(node.get("cllx") or node.get("cllxDm") or "").strip()
                            if node_cllx == _CLLX:
                                slot_qm = self._sanitize(node.get("qmPath"))
                                if slot_qm:
                                    sig_oss_path = slot_qm
                                    logger.info("从材料槽位获取 qmPath: %s", slot_qm)
                                    break
                            queue.extend(v for v in node.values() if isinstance(v, (dict, list)))
                        elif isinstance(node, list):
                            queue.extend(node)
            except Exception:
                logger.debug("从 layyxq 获取 qmPath 失败，使用空值")
    ...
    async def _fetch_signature_info(self: Any) -> tuple[str | None, str | None]:
        ...
        if not items:
            ...
            return None, None
        # 遍历所有签名记录，找到有有效 osspath 或 url 的项
        download_url: str | None = None
        oss_path: str | None = None
        for item in items:
            if not isinstance(item, dict):
                continue
            logger.info("dzqmList item keys=%s", list(item.keys()))
