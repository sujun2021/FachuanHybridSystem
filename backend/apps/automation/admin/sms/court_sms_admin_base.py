...
class CourtSMSAdminBase(admin.ModelAdmin):
    ...
    def recommended_cases_display(self, obj: CourtSMS) -> SafeString:
        ...
        return format_html(
            ...
            "  return;"
            " }}"
            " var cardW=360,gap=10,cw=box.clientWidth||700,cols=Math.max(1,Math.floor((cw+gap)/(cardW+gap)));"
            ...
            "  html+="
            "   '<div class=\"rec-card\" data-id=\"'+r.id+'\" style=\"min-width:0;max-width:'+cardW+'px;border:2px solid '+borderColor+';border-radius:6px;padding:12px 14px;background:#fff;box-sizing:border-box;position:relative;\">'"
