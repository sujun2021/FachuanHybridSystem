...
class CaseAdminServiceMixin:  # pragma: no cover
    """Admin 层服务工厂 mixin，统一提供 Service 实例获取方法。"""

    def _get_case_admin_service(self) -> CaseAdminService:  # pragma: no cover
        """工厂方法：获取 CaseAdminService 实例（延迟导入避免循环依赖）。"""
        from apps.cases.services.case.case_admin_service import CaseAdminService

        return CaseAdminService()

    def _get_case_chat_service(self) -> CaseChatService:  # pragma: no cover
        """工厂方法：获取 CaseChatService 实例（延迟导入避免循环依赖）。"""
        from apps.cases.services.chat.case_chat_service import CaseChatService

        return CaseChatService()

    def _get_case_material_service(self) -> CaseMaterialService:  # pragma: no cover
    ...
    def _get_case_template_binding_service(self) -> CaseTemplateBindingService:  # pragma: no cover
    ...
    def _get_case_assignment_service(self) -> CaseAssignmentService:  # pragma: no cover
    ...
    def _get_case_folder_binding_service(self) -> Any:  # pragma: no cover
