...
class CasePartyCommandRepo:
    def get_case(self, case_id: int) -> Case | None:  # pragma: no cover
        return Case.objects.select_related("contract").filter(id=case_id).first()

    def get_party(self, party_id: int) -> CaseParty | None:  # pragma: no cover
        return CaseParty.objects.select_related("case", "client").filter(id=party_id).first()

    def get_party_for_update(self, party_id: int) -> CaseParty | None:  # pragma: no cover
        return CaseParty.objects.select_related("case").filter(id=party_id).first()

    def party_exists(self, *, case_id: int, client_id: int) -> bool:  # pragma: no cover
